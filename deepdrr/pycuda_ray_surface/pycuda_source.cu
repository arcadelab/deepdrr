...
__device__ void subtract_ffd(const float *a, const float *b, double *c)
{
    c[0] = a[0]-b[0]; c[1] = a[1]-b[1]; c[2] = a[2]-b[2];
}
__device__ void dot_dfd(const double *a, const float *b, double &c)
{
    c = a[0]*b[0] + a[1]*b[1] + a[2]*b[2];
}
__device__ void dot_ddf(const double *a, const double *b, float &c)
{
    c = static_cast<float>(a[0]*b[0] + a[1]*b[1] + a[2]*b[2]);
}
__device__ void dot_dff(const double *a, const float *b, float &c)
{
    c = static_cast<float>(a[0]*b[0] + a[1]*b[1] + a[2]*b[2]);
}
__device__ void cross_dfd(const double *a, const float *b, double *c)
{
...
/* auxiliary functions */
#if COMPILE_DOUBLE_PRECISION_MOLLER
__device__ double tolerance(const double *d, const float *eAB, const float *eAC)
{
    double scaling = sqrt((d[0]*d[0] + d[1]*d[1] + d[2]*d[2]) *
        ...
        float* __restrict__ distance, int numRays)
{
    ...
    if (i < numRays) {
        float sqSum, delta[3];
        subtract(&rayTo[3*i], &rayFrom[3*i], delta);
        dot(delta, delta, sqSum);
        distance[i] = t[i] * sqrtf(sqSum);
    }
}
        ...
        float* __restrict__ point, int numRays)
{
    ...
    if (i < numRays) {
        for (int j = 0; j < 3; j++) {
            point[3*i+j] = rayFrom[3*i+j] + t[i] * (rayTo[3*i+j] - rayFrom[3*i+j]);
        }
    }
}
        ...
        float &t, float &u, float &v)
{
    ...
    cross_dfd(direction, edge2, avec);
    dot_dfd(avec, edge1, det);
    double epsilon = tolerance(direction, edge1, edge2);
    if (det > epsilon) {
        subtract_ffd(q0, v0, tvec);
        dot_ddf(avec, tvec, u);
        if (u < 0 || u > det)
            return 0;
        cross_dfd(tvec, edge1, bvec);
        dot_ddf(bvec, direction, v);
        if (v < 0 || u + v > det)
            return 0;
    }
    else if (det < -epsilon) {
...
#else
#error "Please provide ALIGN macro definition for your host compiler!"
#endif
...
{
    //This is a recursive function (implemented here as a loop). It sets
    //the parent node bounding box and traverses to the root node.
